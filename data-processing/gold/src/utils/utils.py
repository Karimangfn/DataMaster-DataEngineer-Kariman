...
def add_purchase_month_column(df, date_col="purchase_date"):
    ...
    """
    return df.withColumn("purchase_month", date_format(col(date_col), "yyyy-MM"))

def aggregate_purchase_metrics(df, group_cols):
    """
    Aggregates purchase metrics: total purchases, total revenue, average purchase value,
    and count of high value purchases.
    ...
    return (
        df.groupBy(*group_cols)
          .agg(
              ...
              _sum(expr("CASE WHEN high_value_purchase THEN 1 ELSE 0 END"))
